compare_tertiary_bcr_and_single_cell_bcr_extract_cdrs = function(sc.bcell.meta, ter_bcr_data,
                                                                 nt_or_aa = "nt", 
  ...
  if (nt_or_aa == "aa"){
    sc.bcell.meta$cdr3_trim = substr(sc.bcell.meta$cdr3, 2, nchar(sc.bcell.meta$cdr3)-1)
  } else if (nt_or_aa == "nt"){
    sc.bcell.meta$cdr3_nt_trim = substr(sc.bcell.meta$cdr3_nt, 4, nchar(sc.bcell.meta$cdr3_nt)-3)
  }
  ...
  ##In this extraction version, the BCR in the single cell bcr data where the highest similarity is reached will be extracted 
  if (nt_or_aa == "aa"){
    bcr_highest_similarity = c()
    sc.index = c()
    for (ter_bcr in ter_bcr_data$junction_aa) {
      similarity = sapply(sc.bcell.meta$cdr3_trim, string_similarity, str2 = ter_bcr)
      ## Get the highest similarity score and the corresponding index in the sc.bcell.meta
      ## The highest similarity score should be higher than the similarity_threshold
      if (max(similarity) >= similarity_threshold){
        bcr_highest_similarity = c(bcr_highest_similarity, max(similarity))
        highest_similarity_index = which.max(similarity)
        sc.index = c(sc.index, highest_similarity_index)
      } else {
        bcr_highest_similarity = c(bcr_highest_similarity, 0)
        sc.index = c(sc.index, 0)
      }
    }
  } else if (nt_or_aa == "nt"){
    bcr_highest_similarity = c()
    ...
    for (ter_bcr in ter_bcr_data$cdr3) {
      similarity = sapply(sc.bcell.meta$cdr3_nt_trim, string_similarity, str2 = ter_bcr)
      ## Get the highest similarity score and the corresponding index in the sc.bcell.meta
      ## The highest similarity score should be higher than the similarity_threshold
      if (max(similarity) >= similarity_threshold){
        bcr_highest_similarity = c(bcr_highest_similarity, max(similarity))
  ...
  ##Get the highest similarity score based on sc.index to extract cdrs in the single cell bcr data
  ##Make sure the sc.index is unique
  sc.index = unique(sc.index)
  sc.bcell.meta = sc.bcell.meta[sc.index,]
  ##Return the new_ter_bcr_data and the sc.bcell.meta
  return(list(new_ter_bcr_data, sc.bcell.meta))
}
